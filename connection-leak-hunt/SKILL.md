...
Convert the slope into urgency. Default container `nofile=65536`; adjust if you've raised it.

| Slope (FDs/min) | Time-to-exhaustion at `nofile=65536` | Action |
|---|---|---|
...
| 10–100 | hours | same-day fix; mitigate via scheduled restart (`kubectl rollout restart`) every N hours |
| > 100 | minutes | page; rollback the offending deploy or run a restart loop while patching |
...
## Not a leak — common look-alikes

Before opening a sub-skill, rule out these four. They produce climbing-pool or climbing-FD symptoms that are not leaks.
...
# fd-near-limit — last-chance backstop
- alert: PodFDNearLimit
  expr: container_file_descriptors / container_file_descriptors_limit > 0.8
  for: 5m
```
...
Sub-skills add domain-specific alerts (pool-state, channel state, TaskManager metrics).
